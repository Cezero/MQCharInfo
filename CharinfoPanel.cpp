...
static void DrawPeerData(const charinfo::CharinfoPeer& peer)
{
	...
	if (ImGui::TreeNodeEx("Zone", ImGuiTreeNodeFlags_SpanFullWidth)) {
		...
		ImGui::Text("Distance");
		ImGui::TableSetColumnIndex(1);
		if (peer.zone.distance >= 0)
			ImGui::Text("%.1f", peer.zone.distance);
		else
			ImGui::TextUnformatted("—");
		ImGui::TreePop();
	}

	// State (same as Lua State[])
	{
		std::string stateDisplay;
		for (const auto& s : peer.state) {
	...
	// BuffState (same as Lua BuffState[])
	{
		std::string buffDisplay;
		for (const auto& s : peer.buff_state) {
	...
	if (ImGui::TreeNodeEx("Gems", ImGuiTreeNodeFlags_SpanFullWidth)) {
		for (size_t i = 0; i < peer.gems.size(); ++i) {
			...
			ImGui::Text("[%zu]", i + 1);
			ImGui::TableSetColumnIndex(1);
			ImGui::Text("%d %s", peer.gems[i].id, peer.gems[i].name.c_str());
		}
		ImGui::TreePop();
	}
	...
	if (ImGui::TreeNodeEx("Buff", ImGuiTreeNodeFlags_SpanFullWidth)) {
		for (size_t i = 0; i < peer.buff.size(); ++i) {
			...
			ImGui::TableNextRow();
			ImGui::TableSetColumnIndex(0);
			ImGui::Text("[%zu] Duration", i + 1);
			ImGui::TableSetColumnIndex(1);
			ImGui::Text("%d", entry.duration);
			ImGui::TableNextRow();
			ImGui::TableSetColumnIndex(0);
			ImGui::Text("[%zu] Spell Name", i + 1);
			ImGui::TableSetColumnIndex(1);
			ImGui::Text("%s", entry.spell.name.c_str());
			ImGui::TableNextRow();
			ImGui::TableSetColumnIndex(0);
			ImGui::Text("[%zu] Spell ID", i + 1);
			ImGui::TableSetColumnIndex(1);
			ImGui::Text("%d", entry.spell.id);
			ImGui::TableNextRow();
			ImGui::TableSetColumnIndex(0);
			ImGui::Text("[%zu] Spell Category", i + 1);
			ImGui::TableSetColumnIndex(1);
			ImGui::Text("%d", entry.spell.category);
			ImGui::TableNextRow();
			ImGui::TableSetColumnIndex(0);
			ImGui::Text("[%zu] Spell Level", i + 1);
			ImGui::TableSetColumnIndex(1);
			ImGui::Text("%d", entry.spell.level);
		}
		ImGui::TreePop();
	}
...
void DrawCharinfoPanel()
{
	...
	for (const std::string& name : names) {
		...
		if (it == peers.end())
			continue;
		const std::shared_ptr<charinfo::CharinfoPeer>& peerPtr = it->second;
		if (!peerPtr)
			continue;
